...
- `op_art`: high-contrast math, optical, kaleidoscope, ray, bubble, glass, and caustic patterns.
- `scene_cards` / `text_safe`: plates and frames with calmer center areas for text or no-HUD scenes.
- `glitch`: CRT, oscilloscope, scan, static, signal, transmission, and data-rain patterns.
- `maps`: contour, circuit, cellular, maze, constellation, radar, magnetic-field, city-grid, and sigil-like patterns.
- `weather`: rain, fog, heat shimmer, and static.
- `ornamental`: lace, damask, woven, herringbone, ivy, tarot, and geometric motifs.
